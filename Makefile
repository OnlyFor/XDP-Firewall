...
xdpfw: utils libxdp $(OBJS)
	mkdir -p $(BUILDDIR)/
	$(CC) -g0 -Ofast -march=$(MCPU) -mcpu=$(MCPU) -mtune=$(MCPU) -flto $(LDFLAGS) $(INCS) -o $(BUILDDIR)/$(XDPFWOUT) $(LIBBPFOBJS) $(LIBXDPOBJS) $(OBJS) $(SRCDIR)/$(XDPFWSRC)

# XDP program chain.
xdpfw_filter:
	mkdir -p $(BUILDDIR)/
	$(CC) $(INCS) -D__BPF__ -D __BPF_TRACING__ -Wno-unused-value -Wno-pointer-sign -Wno-compare-distinct-pointer-types -g0 -Ofast -march=$(MCPU) -mcpu=$(MCPU) -mtune=$(MCPU) -flto -emit-llvm -c -g -o $(BUILDDIR)/$(XDPPROGLL) $(SRCDIR)/$(XDPPROGSRC)
	$(LLC) -march=bpf -filetype=obj -o $(BUILDDIR)/$(XDPPROGOBJ) $(BUILDDIR)/$(XDPPROGLL)
	
# Utils chain.
utils:
	mkdir -p $(BUILDDIR)/
	$(CC) -g0 -Ofast -march=$(MCPU) -mcpu=$(MCPU) -mtune=$(MCPU) -flto -c -o $(BUILDDIR)/$(CONFIGOBJ) $(SRCDIR)/$(CONFIGSRC)
	$(CC) -g0 -Ofast -march=$(MCPU) -mcpu=$(MCPU) -mtune=$(MCPU) -flto -c -o $(BUILDDIR)/$(CMDLINEOBJ) $(SRCDIR)/$(CMDLINESRC)
